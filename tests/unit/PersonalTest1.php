...
class PersonalTest1 extends TestCase
{
    private $pdo;
    ...
    private function limpiarTablas()
    {
        ...
        $this->pdo->exec("DELETE FROM Personal");
    }
    public function testRegistrarPersonal()
    {
        ...
        // Recuperar el personal por su ID
        $stmt = $this->pdo->prepare("SELECT * FROM Personal WHERE idPersonal = :idPersonal");
        $stmt->execute([':idPersonal' => $idPersonal]);
        $result = $stmt->fetch(PDO::FETCH_ASSOC);

        // Afirmaciones para validar que el personal fue registrado correctamente
